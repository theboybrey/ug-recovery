...
const ReleasedItemsPage = () => {
  ...
  // Filter released claims for officer's points
  const releasedClaims = claimRequests.filter(
    (claim) =>
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-success to-success-700 rounded-xl p-6 text-white">
          <h1 className="text-2xl font-bold mb-2">Released Items</h1>
          <p className="opacity-90">
            View items that have been claimed and released from your assigned collection points
          </p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-surface border-b border-card-border">
                <tr>
                  ...
                  <th className="text-left p-4 font-semibold text-text">Claimant</th>
                  <th className="text-left p-4 font-semibold text-text">Released Date</th>
                ...
                {releasedClaims.length === 0 ? (
                  ...
                  releasedClaims.map((claim) => (
                    <tr key={claim.id} className="border-b border-card-border hover:bg-surface transition-colors">
                      ...
                      <td className="p-4">{claim.appointmentDate || claim.reviewedAt}</td>
                      <td className="p-4">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-card-bg rounded-xl p-6 w-full max-w-md mx-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-text">Released Item Details</h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="font-medium text-text">{selectedClaim.claimantName}</p>
                </div>
                <div>
                  <span className="text-text-muted">Released Date:</span>
                  <p className="font-medium text-text">{selectedClaim.appointmentDate || selectedClaim.reviewedAt}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-text">{selectedClaim.reviewedBy || "-"}</p>
                </div>
                <div>
...
export default ReleasedItemsPage;
